...
class AdminFeildController extends Controller
{
    ...
    public function index(Admin $admin)
    {
       ...
       $feilds = AdminMajor::where('admin_id',$admin->id)->get();
    ...
    public function store(Request $request,Admin $admin)
    {
        ...
        $fd = AdminMajor::where('admin_id',$admin->id)->where('feild',$request->feild)->first();
        if($fd==null)
        {
    ...
    public function edit(Admin $admin, AdminMajor $feild)
    {
        //اجازه دسترسی
        config(['auth.defaults.guard' => 'admin']);
    ...
    public function update(Request $request, Admin $admin, AdminMajor $feild)
    {
        //اجازه دسترسی
        config(['auth.defaults.guard' => 'admin']);
        $this->authorize('admins.edit');
        ...
        $fd = AdminMajor::where('admin_id',$admin->id)->where('feild',$request->feild)->where('id','<>',$feild->id)->first();
        if($fd==null)
        {
            Validator::extend('unique_validator',function(){return true;});
        }
    ...
    public function destroy(Admin $admin, AdminMajor $feild)
    {
        //اجازه دسترسی
        config(['auth.defaults.guard' => 'admin']);
