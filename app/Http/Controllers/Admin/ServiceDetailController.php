...
class ServiceDetailController extends Controller
{
    ...
    public function index()
    {
       ...
       $details =  ServiceDetail::orderBy('name','asc')
       ->withTrashed()
       ->orderBy('name','asc')
       ->filter()
       ->paginate(10)
    ...
    public function create()
    {
        ...
        $branches =Branch::where('status',Status::Active)->orderBy('name','asc')->get();
        return view('admin.details.create',compact('allservices','doctors','advisers','branches'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if($request->status == Status::Active || $request->status == Status::Deactive)
        {
            ...
            $detail->advisers()->sync($request->advisers);
            $detail->branches()->sync($request->branches);
        }
    ...
    public function edit(ServiceDetail $detail)
    {
        ...
        $detail = ServiceDetail::with('doctors','advisers','branches')->find($detail->id);
        $allservices = Service::orderBy('name','asc')->get();
        $branches =Branch::where('status',Status::Active)->orderBy('name','asc')->get();
        return view('admin.details.edit',compact('allservices','detail','doctors','advisers','branches'));
    }
