                                                    ...
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            <form action="{{ route('admin.gallery.update',$gallery) }}"  method="POST" class="d-inline" id="editform{{ $gallery->id }}" enctype="multipart/form-data">
                                                                ...
                                                                <div class="row mt-2">
                                                                    <div class="col-12">
                                                                        <select name="services[]" id="services{{$gallery->id}}" class="select2 select2-multiple text-right IRANYekanRegular" multiple="multiple" multiple data-placeholder="انتخاب سرویس ها...">
                                                                            @foreach($serviceDetails as $service)
                                                                                <option value="{{ $service->id }}" @if(in_array($service->id,$gallery->services->pluck('id')->toArray())) selected @endif>{{ $service->name }}</option>
                                                                            @endforeach
                                                                        </select>
                                                                    </div>
                                                                </div>
