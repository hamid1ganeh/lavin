                                ...
                                <div class="card card-body filter">
                                    <form id="filter-form">
                                        <div class="row">
                                            ...
                                            <div class="form-group justify-content-center col-6">
                                                <label for="priorities-filter" class="control-label IRANYekanRegular">اولویت</label>
                                                <select name="priorities[]" id="priorities-filter" class="form-control select2 select2-multiple text-right IRANYekanRegular" multiple="multiple" multiple data-placeholder="...  اولیت های مورد نظر را انتخاب نمایید">
                                                    <option value="{{ App\Enums\TicketPriority::Low }}" @if(request('priorities')!=null) {{ in_array(App\Enums\TicketPriority::Low,request('priorities'))?'selected':'' }} @endif>کم</option>
                                                    <option value="{{ App\Enums\TicketPriority::Medium }}" @if(request('priorities')!=null) {{ in_array(App\Enums\TicketPriority::Medium,request('priorities'))?'selected':'' }} @endif>متوسط</option>
                                                    <option value="{{ App\Enums\TicketPriority::High }}" @if(request('priorities')!=null) {{ in_array(App\Enums\TicketPriority::High,request('priorities'))?'selected':'' }} @endif>زیاد</option>
                                                </select>
                                            </div>

                                            <div class="form-group justify-content-center col-6">
                                                <label for="status" class="control-label IRANYekanRegular">وضعیت</label>
                                                <select name="status[]" id="status-filter" class="form-control select2 select2-multiple text-right IRANYekanRegular" multiple="multiple" multiple data-placeholder="...  اولیت های مورد نظر را انتخاب نمایید">
                                                    <option value="{{ App\Enums\TicketStatus::Waiting }}" @if(request('status')!=null) {{ in_array(App\Enums\TicketStatus::Waiting,request('status'))?'selected':'' }} @endif>رانتظار پاسخ</option>
                                                    <option value="{{ App\Enums\TicketStatus::Pending }}" @if(request('status')!=null) {{ in_array(App\Enums\TicketStatus::Pending,request('status'))?'selected':'' }} @endif>درحال بررسی</option>
                                                    <option value="{{ App\Enums\TicketStatus::Answerd }}" @if(request('status')!=null) {{ in_array(App\Enums\TicketStatus::Answerd,request('status'))?'selected':'' }} @endif>پاسخ داده شده</option>
                                                    <option value="{{ App\Enums\TicketStatus::Close }}" @if(request('status')!=null) {{ in_array(App\Enums\TicketStatus::Close,request('status'))?'selected':'' }} @endif>بسته شده</option>
                                                </select>
                                            </div>

                                            <div class="form-group justify-content-center col-6">
                                                <label for="department" class="control-label IRANYekanRegular">واحد</label>
                                                <select name="departments[]" id="departments-filter" class="form-control select2 select2-multiple text-right IRANYekanRegular" multiple="multiple" multiple data-placeholder="...  واحدهای مورد نظر را انتخاب نمایید">
                                                    @foreach($departments as $department)
                                                    <option value="{{ $department->id }}"   @if(request('departments')!=null) {{ in_array($department->id,request('departments'))?'selected':'' }} @endif>{{ $department->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        ...
                                        <div class="form-group col-12 d-flex justify-content-center mt-3">
                                            ...
                                            <script>
                                                function reset()
                                                {
                                                    ...
                                                    $("#priorities-filter").val(null).trigger("change");
                                                    $("#status-filter").val(null).trigger("change");
                                                    $("#departments-filter").val(null).trigger("change");
                                                }
                                            </script>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <strong class="IRANYekanRegular">
                                                    {{ $ticket->user->firstname.' '.$ticket->user->lastname.' ('.$ticket->user->mobile.')' }}
                                                </strong>
                                                @endif
                                            </td>
